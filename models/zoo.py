...
class CPrompt(nn.Module):
	def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768):
		super().__init__()
		self.task_count = 0
		print(" in CPrompt prompt")
...
class ViTZoo(nn.Module):
	def __init__(self, num_classes=10, pt=False, prompt_flag=False, prompt_param=None):
		...
		elif self.prompt_flag == 'cprompt':
			self.prompt = CPrompt(768, prompt_param[0], prompt_param[1])
		else:
			self.prompt = None
	...
	def forward(self, x, pen=False, train=False):
		if train:
			...
			pre_logits = out.clone()
			if not pen:
				out = self.last(out)
			if self.prompt is not None and train:
				return out, prompt_loss, pre_logits
		else:
			with torch.no_grad():
				out, _ = self.feat(x, train=train, task_id=self.task_id)
				out = out[:,0,:]
				if self.prompt is not None:
					out, prompt_loss = self.feat(x, prompt=self.prompt, q=out, train=train, task_id=self.task_id)
					out = out[:,0,:]	
				out = out.view(out.size(0), -1)
				if not pen:
					out = self.last(out)
