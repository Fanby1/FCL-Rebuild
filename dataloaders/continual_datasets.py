...
class Imagenet_R(torch.utils.data.Dataset):
	def __init__(self, root, train=True, transform=None, target_transform=None, download=False):		
		...
		with ThreadPoolExecutor(max_workers=cpu_count) as executor:
			futures = []
			for idx, folder in enumerate(folders):
				futures.append(executor.submit(process_file, fpath, folder, idx))
...
class my_data_for_visual(torch.utils.data.Dataset):
	def __init__(self, root, train=True, transform=None, target_transform=None, domain=1):
		self.root = os.path.expanduser(root)
		self.transform = transform
		self.target_transform=target_transform
		self.train = train
		...
		fpath = os.path.join(root,'my_data/')
